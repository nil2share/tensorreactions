local tbl = 
{
	...
	[44] = 
	{
		
		{
			...
			enabled = true,
			execute = "if NilsReactionCore.Hotbar.Toggles.Plunge.Execute(NilsReactionCore.params.off, NilsReactionCore.params.isNotTimeline, NilsReactionCore.params.on, 3000) then self.used = true end\n",
			executeType = 2,
			lastUse = 0,
			loop = false,
			luaNeedsWeaveWindow = false,
	...
	[60] = 
	{
		
		{
			...
			enabled = true,
			execute = "-- Requires Argus for this, no easy way I can find to detect if you are in in the first or second set\nif NilsReactionCore.Argus.EntityInUnavoidableAOEs(Player) then\n   NilsReactionCore.Hotbar.Darkknight.DarkMind()\n   NilsReactionCore.Hotbar.Toggles.Plunge.Execute(NilsReactionCore.params.off, NilsReactionCore.params.isNotTimeline, NilsReactionCore.params.on, 3000)\n   if NilsReactionCore.Hotbar.Darkknight.TheBlackestNight() then self.used = true end\nend",
			executeType = 2,
			lastUse = 0,
			loop = false,
			luaNeedsWeaveWindow = false,
	...
	[66] = 
	{
		
		{
			...
			enabled = true,
			execute = "local target = Player:GetTarget()\nif target ~= nil and table.valid(target) and target.attackable and target.hp.percent < 2 then\n  NilsReactionCore.Logic.Toggles.BurnBossOn(NilsReactionCore.params.isTimeline)\n  NilsReactionCore.Logic.Toggles.BurnGapClosersOn(NilsReactionCore.params.isTimeline)\n  self.used = true\nend",
			executeType = 2,
			lastUse = 0,
			loop = false,
			luaNeedsWeaveWindow = false,
